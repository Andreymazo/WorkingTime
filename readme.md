Я потерял секретный ключ, надо заново все скопировать в этом проекте и сделать новый с ключом, сейчас сначала подтягивал с соседнего проекта workingtimemanage интерпритатор и вьюху смотрит туда и вообще все становится непонятно, а тот проект вообще не работал никогда. Ну а потом просто стал требовать энве файл, которого нет.
